---
title: "Análise biocontrole de Hemileia vastatrix"
author: "Nívia"
format: html
editor: visual
---
...
Por meio deste gráfico, é possível ter uma visão mais clara e distinguir quais tratamentos possibilitaram uma maior severidade da doença ao final do experimento. Considerando apenas a análise visual, os tratamentos AT, HI, BI e 58 são os piores no controle da doença, pois apresentaram maior severidade final.
...
**Arrumando os dados:** como o data frame tem mais de uma variável resposta, fez-se necessário selecionar as colunas desejadas para trabalhar nos dados de incidência e também filtrar as células com ausência de dados para evitar erros durante as análises.
...
**Gráfico de colunas com média dos dados:** os dados foram agrupados pelos tratamentos (trat) e a média e o desvio padrão da incidência foram calculados. Após isso, foi feita a plotagem do gráfico de colunas novamente com a média dos dados e contendo a barra de erro.
...
Baseado nos valores de p (p-valor) na tabela de anova, interpretou-se que, como o valor de p para os tratamentos foram menores que o nível de significância (0,05), deve-se rejeitar a hipótese nula, uma vez que há evidência estatística de que pelo menos um dos tratamentos diferencia-se na incidência da doença em comparação aos demais.
...
**Gráfico de colunas com média dos dados:** os valores ausentes foram filtrados, os dados de incidência agrupados pelos tratamentos (trat) e a média e o desvio padrão da variável resposta foram calculados usando a função `summarise`. Após isso, foi feita a plotagem do gráfico de colunas novamente com a média e realizando a inserção da barra de erro.
...
## Correlação entre as variáveis incidência e altura de planta
...
**Visualização e análise exploratória:** plotou-se os dados em um gráfico de dispersão usando a função `geom_point` contendo em sua estética ambas as variáveis resposta. A camada com a função `geom_smooth` foi utilizada para criar uma linha de tendência. Dentro dessa função foi especificado que as barras de erro não serão exibidas no gráfico (se = F") e que o método de regressão linear deve usado para ajustar a linha (method = "lm").
